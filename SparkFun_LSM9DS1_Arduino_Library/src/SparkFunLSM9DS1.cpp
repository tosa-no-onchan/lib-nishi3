...
uint16_t LSM9DS1::begin(uint8_t agAddress, uint8_t mAddress, TwoWire &wirePort)
{
	...
	// test by nishi
	Serial.println("LSM9DS1::begin() : #1");

	uint8_t xgTest = xgReadByte(WHO_AM_I_XG);	// Read the accel/gyro WHO_AM_I
	#if defined(USE_NISI_MAG)
		uint8_t mTest = mReadByte(WHO_AM_I_M);		// Read the magnet WHO_AM_I
		uint16_t whoAmICombined = (xgTest << 8) | mTest;
		if (whoAmICombined != ((WHO_AM_I_AG_RSP << 8) | WHO_AM_I_M_RSP))
			return 0;
	#else
		uint16_t whoAmICombined = (xgTest << 8);
		if (whoAmICombined != (WHO_AM_I_AG_RSP << 8))
			return 0;
	#endif

	Serial.println("LSM9DS1::begin() : #2");
...
uint16_t LSM9DS1::beginSPI(uint8_t ag_CS_pin, uint8_t m_CS_pin)
{
	...
	// test by nishi
	Serial.println("LSM9DS1::beginSPI() : #1");
	...
	int i;
		...
		i=5;
		while(i>=0){
			whoAmI = mReadByte(WHO_AM_I_M);		// Read the magnet WHO_AM_I
	...
	i=5;
	while(i>=0){
		whoAmI = xgReadByte(WHO_AM_I_XG);	// Read the accel/gyro WHO_AM_I
	...
	if(whoAmI != WHO_AM_I_AG_RSP){
		Serial.print("AG_whoAmI=");
		Serial.println(whoAmI,HEX);
		return 0;
	}

	Serial.println("LSM9DS1::beginSPI() : #2");
	...
	//#define SPI_CLOCK_DIV128  0x04fc1001 //125 KHz
  	SPI.setClockDivider( SPI_CLOCK_DIV2 ); // changed by nishi 2022.5.2
...
uint8_t LSM9DS1::xgReadByte(uint8_t subAddress)
{
	...
	if (settings.device.commInterface == IMU_MODE_I2C)
		...
		// test by nishi
		//Serial.println("LSM9DS1::xgReadByte() : #1");
		//Serial.print("_xgAddress:");
		//Serial.print(_xgAddress);
...
void LSM9DS1::initSPI()
{
	...
	SPI.setDataMode( SPI_MODE0 );   // changed by nishi
	//SPI.setDataMode( SPI_MODE3 );   // changed by nishi
	...
	//#define SPI_CLOCK_DIV128  0x04fc1001 //125 KHz
  	...
  	//SPI.setClockDivider( SPI_CLOCK_DIV8 ); // changed by nishi 2022.5.2
  	//SPI.setClockDivider( SPI_CLOCK_DIV16 ); // changed by nishi 2022.5.2
  	SPI.setClockDivider( SPI_CLOCK_DIV128 ); // changed by nishi 2022.5.2
